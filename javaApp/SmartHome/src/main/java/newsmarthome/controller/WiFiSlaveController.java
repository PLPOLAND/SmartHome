...
public class WiFiSlaveController {
	...
	Logger logger = LoggerFactory.getLogger(WiFiSlaveController.class);
	
	
    WiFiSlaveController(@Autowired UsersDAO users, @Autowired SystemDAO systemDAO, @Autowired HardwareFactory hardwareFactory, @Autowired WifiSlaveRepository wifiSlaveRepository) {
        this.users = users;
